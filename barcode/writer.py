...
if TYPE_CHECKING:
    # Typevar defaults (PEP 696) only exist at runtime on Python 3.13+, so use
    # the typing_extensions variant for type-checkers only; this way it is not
    # a runtime dependency.
    from typing_extensions import TypeVar

    T_Output = TypeVar("T_Output", default=bytes)
else:
...
class BaseWriter(Generic[T_Output]):
    ...
    def render(self, code: list[str]) -> T_Output:
    ...
    def write(self, content: T_Output, fp: BinaryIO) -> None:
        raise NotImplementedError
...
if Image is None:
    if TYPE_CHECKING:
        # Keep this stub's signatures in sync with the real class below. It
        # exists so that type-checkers see a single, always-defined class
        # regardless of whether Pillow is installed.

        class ImageWriter(BaseWriter["T_Image"]):
            def __init__(
                self,
                format: str = "PNG",
                mode: str = "RGB",
    ...
    class ImageWriter(BaseWriter["T_Image"]):  # type: ignore[no-redef]
        format: str
        mode: str
        dpi: int
